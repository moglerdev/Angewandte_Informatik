...
public final class NotenstatistikBonus {
     ...
     */
    public static Integer evalInput() {
        ...
        try {
            vorkommaNote = Integer.parseInt(splitNote[0].substring(0, 1));
            if(splitNote[0].length() > 1){
                if (splitNote[0].endsWith("+")) {
                    if(vorkommaNote > 1){
                        nachkommaNote = 7;
                        --vorkommaNote;
                    } else {
                ...
                } else if (splitNote[0].endsWith("-")) {
                    nachkommaNote = 3;
                } else {
                    write("Note %s wird wegen Formatfehler ignoriert!%n", stringNote);
                    throw new InputMismatchException();
                }
            } else if (splitNote.length > 1) {
                if(splitNote[1].length() > 1){
                    write("Note %s wird wegen Formatfehler ignoriert!%n", stringNote);
                    throw new InputMismatchException();
                }
    ...
    public static void handleInput() {
        ...
        Integer cntNoten = 0;
        Integer cntProofedNoten = 0;
        Integer bestNote = 50;
        Integer badestNote = 10;
