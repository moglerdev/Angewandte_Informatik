...
int main()
{
    ...
    while (true)
    {
        try
        {
            ...
            {
                // neue Fachnote in Notenliste eintragen:
                std::shared_ptr<fachnote> f = fachnote::new_instance(fach, benotung{note, note2});
                liste.push_front(f);
            }
        }
    ...
    std::size_t max_laenge = std::accumulate(liste.begin(), liste.end(), 0, [](size_t a, std::shared_ptr<fachnote> b)
    {
        if (a < b->fach.length())
        {
